document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Clipboard Buttons (inline "Copied ✓" feedback) ---
    const copyButton = document.getElementById('copy-button');
    if (copyButton) {
        copyButton.addEventListener('click', () => {
            copyWithFeedback(copyButton, document.getElementById('generated-command').innerText);
        });
    }
    ...
    if (copyAdditionalButton) {
        copyAdditionalButton.addEventListener('click', () => {
            copyWithFeedback(copyAdditionalButton, document.getElementById('additional-command').innerText);
        });
    }
    ...
    if (rdbmsSelect && dburlField) {
        ...
        // Populate the default JDBC URL on load, then re-render so the initial output
        // reflects the pre-selected RDBMS (the form's live listener was attached by setupForm).
        if (rdbmsSelect.value) {
            updateJdbcUrl();
            const controllerForm = document.getElementById('controller-form');
            if (controllerForm) controllerForm.dispatchEvent(new Event('change', { bubbles: true }));
        }
    }
});

// --- Form setup: live rendering (no submit button needed) ---
// `run(isLive)` regenerates the output. isLive=true is used for keystroke/selection
// updates and suppresses blocking validation UI + auto-scroll; isLive=false (Enter
// key / programmatic submit) keeps the original validating behaviour.
function setupForm(form, script, prefix, defaults) {
    const run = (isLive) => {
        if (form.id === 'windows-form') { handleWindowsForm(form, script, prefix, defaults, isLive); return; }
        if (form.id === 'controller-form') { handleControllerForm(form, script, prefix, defaults, isLive); return; }
        handleLinuxForm(form, script, prefix, defaults, isLive);
    };
...
function handleLinuxForm(form, script, prefix, defaults, isLive) {
        ...
        if (!omsServersValue) {
            if (!isLive) showOmsValidationError();
            hideGeneratedOutput();
            return;
        }
        ...
        for (let i = 0; i < elements.length; i++) {
            ...
            // Hidden inputs backing segmented controls carry a value but always report
            // offsetParent === null; gate them on real section visibility instead so
            // they behave exactly like the visible <select> they replaced.
        ...
        if (toggleUserModeChecked && !hasAllDirs) {
            if (!isLive) showDirectoryValidationErrors('When "I want to install in a different folder" is selected, all three directories must have values.');
            hideGeneratedOutput();
            return;
        }
        ...
        if (hasAnyDir && hasAllDirs) {
            ...
            if (!isLive) showDirectoryValidationErrors('If you specify any directory (Install, Config, or Data), all three directories must have values.');
            hideGeneratedOutput();
            return;
        }
        ...
        // Scroll to the generated command (only on explicit submit, not live edits)
        if (!isLive) {
            setTimeout(() => {
...
function handleWindowsForm(form, script, prefix, defaults, isLive) {
...
function handleControllerForm(form, script, prefix, defaults, isLive) {
    ...
    const dbpassValue = dbpassElement ? dbpassElement.value.trim() : 'CHANGE_ME';
    const rdbmsValue = rdbmsElement ? rdbmsElement.value.trim() : '';
...
// True unless an ANCESTOR is display:none. Used for hidden value-carrier inputs
// (segmented controls): the carrier itself is always display:none (UA style for
// input[type=hidden]), so we only care whether its containing section is visible.
...
function initializeButtonSelectors() {
    ...
    document.querySelectorAll('.option-button').forEach(button => {
        button.addEventListener('click', function() {
            ...
            // Update hidden input value and notify listeners (live render, JDBC URL, etc.)
...
// Turns each `.button-group` of `.option-button`s into an ARIA radiogroup with a
...
function copyWithFeedback(button, text) {
    ...
    const restore = () => { if (labelEl && original !== null) labelEl.textContent = original; button.classList.remove('copied'); };
    if (button._copyTimer) clearTimeout(button._copyTimer);
    navigator.clipboard.writeText(text).then(() => {
        if (labelEl) labelEl.textContent = 'Copied ✓';
        button.classList.add('copied');
        button._copyTimer = setTimeout(restore, 2000);
    }, (err) => {
...
// Hide both output panels (used when live input is invalid/incomplete).
function hideGeneratedOutput() {
    const o = document.getElementById('command-output'); if (o) o.style.display = 'none';
    const a = document.getElementById('additional-command-output'); if (a) a.style.display = 'none';
}

let _ariaGroupSeq = 0;
function enhanceOptionGroups() {
    document.querySelectorAll('.button-group').forEach(group => {
        if (group.dataset.ariaEnhanced) return;
        group.dataset.ariaEnhanced = 'true';
        group.setAttribute('role', 'radiogroup');

        // Associate the group's visible label (if any) via aria-labelledby.
        const formGroup = group.closest('.form-group');
        const labelEl = formGroup ? formGroup.querySelector('label') : null;
        if (labelEl) {
            if (!labelEl.id) labelEl.id = 'optgroup-label-' + (++_ariaGroupSeq);
            group.setAttribute('aria-labelledby', labelEl.id);
        }

        const buttons = () => Array.from(group.querySelectorAll('.option-button'));
        ...
        function sync() {
            ...
            btns.forEach(b => {
                ...
                const checked = b.classList.contains('active');
                b.setAttribute('aria-checked', checked ? 'true' : 'false');
                const focusable = checked && !b.disabled;
                b.tabIndex = focusable ? 0 : -1;
                if (focusable) hasFocusable = true;
            });
            // Guarantee one tab stop even if nothing is active yet.
            if (!hasFocusable) {
        ...
        sync();
        // Re-sync after any click (existing handlers toggle the `active` class).
        ...
        group.addEventListener('keydown', (e) => {
            ...
            for (let i = 0; i < btns.length; i++) {
                idx = (idx + step + btns.length) % btns.length;
                if (!btns[idx].disabled) break;
            }
